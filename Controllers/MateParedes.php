...
class MateParedes extends Controller
{
    ...
    public function index()
    {
        ...
        $verificar = $this->model->verificarPermiso($id_user, 'Material_Pared');
    ...
    {  
        ...
        for ($i = 0; $i < count($data); $i++) {
                ...
                $verificarRegistrar = $this->model->verificarPermiso($id_user , 'Registrar_Material_Pared');
                if (!empty($verificarRegistrar) || $id_user == 1) {
                   
                }else{
                    $registrarDisabled = 'disabled';
                }
                $verificarVer= $this->model->verificarPermiso($id_user , 'Ver_Pared_Inactivo');
                ...
                $data[$i]['accion'] = '<div>
                <button class="btn btn-danger mb-2" type="button" '.$verDisabled .'><a class="dropdown text-white" href="MateParedes/inactivo"><i class="fas fa-eye-slash"></i></a></button>
                </div>';
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $reingresarDisabled = '';
                $id_user = $_SESSION['id_usuario'];
                $verificarReingreso = $this->model->verificarPermiso($id_user , 'Reingresar_Material_Pared');
    ...
    private function generarAcciones($MateId, $inactivo = false) {
        ...
        $id_user = $_SESSION['id_usuario'];
        $verificarEditar = $this->model->verificarPermiso($id_user , 'Editar_Material_Pared');
        if (empty($verificarEditar) && $id_user != 1) {
            $editarDisabled = 'disabled';
        }
    
        $verificarEliminar = $this->model->verificarPermiso($id_user, 'Eliminar_Material_Pared');
    ...
    {
        $data = $this->model->getMateParedesInac();
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                $data[$i]['estado'] = '<span class="badge bg-success">Activo</span>';
                $data[$i]['acciones'] = '<div style="display: flex; justify-content: center;">';
