...
class MatePisos extends Controller
{
    ...
    public function index()
    {
        ...
        $verificar = $this->model->verificarPermiso($id_user, 'Material_Piso');
    ...
    {  
        ...
        for ($i = 0; $i < count($data); $i++) {
                ...
                $verificarRegistrar = $this->model->verificarPermiso($id_user , 'Registrar_Material_Piso');
                if (!empty($verificarRegistrar) || $id_user == 1) {
                   
                }else{
                    $registrarDisabled = 'disabled';
                }
                $verificarVer= $this->model->verificarPermiso($id_user , 'Ver_Piso_Inactivo');
                ...
                $data[$i]['accion'] = '<div>
                <button class="btn btn-danger mb-2" type="button" '.$verDisabled .'><a class="dropdown text-white" href="MatePisos/inactivo"><i class="fas fa-eye-slash"></i></a></button>
                </div>';
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $reingresarDisabled = '';
                $id_user = $_SESSION['id_usuario'];
                $verificarReingreso = $this->model->verificarPermiso($id_user , 'Reingresar_Material_Piso');
    ...
    private function generarAcciones($MateId, $inactivo = false) {
        ...
        $id_user = $_SESSION['id_usuario'];
        $verificarEditar = $this->model->verificarPermiso($id_user , 'Editar_Material_Piso');
        if (empty($verificarEditar) && $id_user != 1) {
            $editarDisabled = 'disabled';
        }
    
        $verificarEliminar = $this->model->verificarPermiso($id_user, 'Eliminar_Material_Piso');
    ...
    {
        $data = $this->model->getMatePisosInac();
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                $data[$i]['estado'] = '<span class="badge bg-success">Activo</span>';
                $data[$i]['acciones'] = '<div style="display: flex; justify-content: center;">';
        ...
        if (empty($nombre) || empty($cantidad) || empty($precioBajo) || empty($precioMedio) || empty($precioAlto) || empty($manoObra)) {
            ...
            if ($id == "") {
                ...
                if ($data == "ok") {
                    ...
                    $msg = "El Material Piso ya existe";
                } else {
                    $msg = "Error al registrar el Material Piso";
                }
            } else {
                $totalBajo = ($cantidad * $precioBajo) / 100;
                $totalMedio = ($cantidad * $precioMedio) / 100;
