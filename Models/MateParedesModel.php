...
class MateParedesModel extends Query
{
    ...
    {
        $sql = "SELECT m.*, e.nombre as id_elemento, u.id as id_medida, u.medida FROM materiales m INNER JOIN elementos e INNER JOIN medidas u WHERE e.id_elemento=m.id_elemento and u.id = m.id_medida and m.id_elemento=3 and m.estado = 1";
        $data = $this->selectAll($sql);
        return $data;
    }
    ...
    public function getMateParedesInac()
    {
        $sql = "SELECT m.*, e.nombre as id_elemento, u.id as id_medida, u.medida FROM materiales m INNER JOIN elementos e INNER JOIN medidas u WHERE e.id_elemento=m.id_elemento and u.id = m.id_medida and m.id_elemento=3 and m.estado = 0";
        $data = $this->selectAll($sql);
        return $data;
    }
    public function registrarMatePiso(string $nombre, string $id_medida,string $cantidad, string $precioBajo, string $precioMedio, string $precioAlto, string $totalBajo, string $totalMedio, string $totalAlto, string $id_elemento, string $manoObra)
    {
        ...
        if (empty($existe)) {
            ...
            $datos = array($this->nombre, $this->id_medida, $this->cantidad, $this->precioBajo, $this->precioMedio, $this->precioAlto, $this->totalBajo, $this->totalMedio, $this->totalAlto, $this->id_elemento, $this->manoObra);
